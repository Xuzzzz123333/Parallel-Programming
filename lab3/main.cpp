...
if (argc >= 2)
{
    ...
    {
        cerr << "Usage: " << argv[0] << " serial|simd" << endl;
        return 1;
    }
}

cout << "Hash mode: " << (use_simd ? "SIMD" : "Serial") << endl;
    //下面代码用于测试MD5哈希的正确性
...
vector<string> guess_vec(q.guesses.begin(), q.guesses.end());

if (!use_simd)
{
    // 串行基线版本
    for (const string &pw : guess_vec)
    {
        bit32 state[4];
        MD5Hash(pw, state);
...
{
    // SIMD版本：每4条一组
    for (size_t idx = 0; idx < guess_vec.size(); idx += 4)
    {
        size_t remain = guess_vec.size() - idx;
...
MD5HashSIMD4Ref(
    ...
    states
            ...
            // 如需输出可在这里处理 states[0..3]
        }
        else
        {
            // 尾巴不足4条，回退到串行
            for (size_t j = idx; j < guess_vec.size(); j++)
            {
                bit32 state[4];
                MD5Hash(guess_vec[j], state);
