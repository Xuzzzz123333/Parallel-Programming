...
static vector<string> GenerateCandidatesCPU(const string &prefix, const vector<string> &values)
{
    ...
    {
        guesses.emplace_back(prefix + value);
    }
...
static bool VerifySamples(
    ...
    int sample_count)
{
    int n = min(sample_count, (int)min(values.size(), cpu_guesses.size()));
    for (int i = 0; i < n; i += 1)
    {
        string expected = prefix + values[i];
        if (expected != cpu_guesses[i])
        {
            cerr << "Mismatch at sample " << i << endl;
            cerr << "expected: " << expected << endl;
            cerr << "actual  : " << cpu_guesses[i] << endl;
            return false;
        }
    }
    return true;
...
int main(int argc, char **argv)
{
    ...
    cout << "=== Basic Full PCFG CPU Baseline Experiment ===" << endl;
    cout << "Train/test data: " << data_path << endl;
    cout << "Generate limit: " << generate_limit << endl;
    cout << "Baseline policy: each time pop exactly one PT; its inner value loop is generated on CPU." << endl;
    cout << "No GPU, no multi-PT batch, no overlap, no threshold scheduling." << endl;
    ...
    while (!pq.empty())
    {
        ...
        if (ok)
        {
            guesses = GenerateCandidatesCPU(prefix, *values);
            stats.processed_pt_count += 1;

            if (!sample_checked)
            {
                bool pass = VerifySamples(prefix, *values, guesses, 5);
                cout << "First CPU PT sample check: " << (pass ? "PASSED" : "FAILED") << endl;
                cout << "First CPU PT value count: " << values->size() << endl;
                if (!guesses.empty())
                {
